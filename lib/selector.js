    ...
    exports.not = function(selector){
        ...
        if (typeof fn != 'function') {
            ...
            if (typeof selector == 'string') {
                removes = new Rye(selector)
            } else {
                removes = this.filter(selector)
            }
            fn = function(element){
                return removes.indexOf(element) != -1
            }
        }
        this.elements.forEach(function(element, index){
            fn(element, index) || elements.push(element)
        })
        return create(elements)        
    }
