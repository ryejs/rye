...
Rye.implement(function(exports){
    ...
    exports.show = function(){
        return this.each(function(element){
            style.setCSS(element, 'display', data.get(element, '_display') || 'block')
        })
    }

    exports.hide = function(){
        return this.each(function(element){
            var _display = style.getCSS(element, 'display')
            if (_display !== 'none') {
                data.set(element, '_display', _display)
            }
            style.setCSS(element, 'display', 'none')
        })
    }
    ...
    exports.css = function(property, value){
        if (value == undefined) {
            if (typeof property == 'string') {
                return this.elements[0] && style.getCSS(this.elements[0], property)
            }

            return this.each(function(element){
                for (var key in property) {
                    style.setCSS(element, key, property[key])
                }
            })
        }
        return this.each(function(element){
            style.setCSS(element, property, value)
        })
    }
